...
document.addEventListener("keydown", (event)=>{
    ...
    if (keyCodes.includes(event.code)) {
        ...
        setRandomColors();
    } else if (event.code === "KeyL") for(i = 0; i < colButtons.length; i++){
        colButtons[i].classList.remove("fa-lock-open");
        colButtons[i].classList.add("fa-lock");
    }
    else if (event.code === "KeyU") for(i = 0; i < colButtons.length; i++){
...
document.addEventListener("click", (event)=>{
    ...
    } else if (type === "copy") copyToClipboard(event.target.textContent);
});
...
function setRandomColors(isInitial) {
    ...
    columns.forEach((col, index)=>{
        ...
        if (colorMatch === true) colorMatchBox.textContent = "solid match";
        else colorMatchBox.textContent = "approx. match";
...
function getColorsFromHash() {
    if (document.location.hash.length > 1) return document.location.hash.substring(1).split("-").map((color)=>"#" + color);
    return [];
}
